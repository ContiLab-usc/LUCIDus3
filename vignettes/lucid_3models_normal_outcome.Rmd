...
plus the `sig` column: `"*"` marks a row whose interval excludes 0, i.e. an
effect the bootstrap distinguishes from no effect at all. With only `R = 5`
replicates here the intervals
themselves are not trustworthy -- this is a code-pattern demonstration, not a
real inference; a real analysis needs `R` in the hundreds, as noted in
section 3's hyperparameter guide.
...
do.call(rbind, lapply(c(0, 0.5, 2, 10), sweep_rho_z))
```
